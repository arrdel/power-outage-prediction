...
def plot_outages_on_map_us(start_time, end_time, data_directory="./data/eaglei_data"):
    """
    ...
    if not os.path.exists(file_path):
        raise FileNotFoundError(
            f"The file {file_name} does not exist in the directory {data_directory}."
        )
    ...
    filtered_df = df[
        (df["run_start_time"] >= start_time) & (df["run_start_time"] <= end_time)
    ]

    grouped = filtered_df.groupby("state").agg({"customers_out": "sum"}).reset_index()
    ...
    fig.update_layout(
        geo=dict(lakecolor="lightblue", showlakes=True),
        title_x=0.4,  # Center the title
        margin=dict(
            l=0, r=0, t=50, b=0
        ),  # Minimize space between colorbar and map figure
        width=1000,
        height=600,  # Set figure size to (10, 6)
        title_font=dict(
            size=20, family="Arial, bold"
        ),  # Set figure title font size and make it bold
...
def plot_outages_on_map_state(
    start_time, end_time, data_directory="./data/eaglei_data", state="Texas"
):
    """
    ...
    filtered_df = df[
        (df["run_start_time"] >= start_time)
        & (df["run_start_time"] <= end_time)
        & (df["state"] == state)
    ]

    grouped = (
        filtered_df.groupby(["county", "fips_code"])
    ...
    grouped["fips_code"] = grouped["fips_code"].astype(
        str
    ...
    county_plot = counties.plot(
        column="customers_out",  # Data to visualize
    ...
    sm = plt.cm.ScalarMappable(
        cmap=custom_cmap, norm=Normalize(vmin=0, vmax=counties["customers_out"].max())
    )
...
def plot_outages_ts_states(
    start_time, end_time, states, data_directory="./data/eaglei_data"
):
    """
    ...
    for state in states:
        ...
        df_state_grouped = (
            df_state.groupby("run_start_time")["customers_out"].sum().reset_index()
        )

        # Plot the time history of total customers out
        plt.plot(
            df_state_grouped["run_start_time"],
            df_state_grouped["customers_out"],
            linestyle="-",
            label=state,
        )
...
def plot_outages_ts_years(state, years, data_directory="./data/eaglei_data"):
    ...
    for year in years:
        if year not in valid_years:
            raise ValueError(
                f"Invalid year: {year}. Year must be between 2014 and 2023."
            )
    ...
    for year in years:
        ...
        df_state["run_start_time"] = df_state["run_start_time"].apply(
...
def barchart_events_by_state(df, state, event_types, all_years=True, which_year=None):
    ...
    for i, event in enumerate(event_types):
        ...
        monthly_data = (
            event_data.groupby("MONTH_NAME").size().reset_index(name="Event Count")
        )

        monthly_data["MONTH_SORT"] = pd.to_datetime(
            monthly_data["MONTH_NAME"], format="%B"
        )
        monthly_data = monthly_data.sort_values(by="MONTH_SORT")

        # Plot the data
        axes[i].bar(
            monthly_data["MONTH_NAME"], monthly_data["Event Count"], color="skyblue"
        )
        ...
        axes[i].set_xticklabels(
            monthly_data["MONTH_NAME"].unique(), rotation=45, ha="right", fontsize=15
        )
...
def make_ts_events(
    ...
    end_date = datetime(
        end_year, end_month, end_day, 23, 45
    )  # Include the last time interval
    ...
    df["END_DATETIME"] = pd.to_datetime(
        df["END_YEARMONTH"].astype(str)
        + df["END_DAY"].astype(str).str.zfill(2)
        + df["END_TIME"].astype(str).str.zfill(4),
        format="%Y%m%d%H%M",
    )
    ...
    cols_order = ["YEAR", "MONTH", "DAY", "time"] + [
        col for col in new_df.columns if col not in ["YEAR", "MONTH", "DAY", "time"]
    ]
...
def combine_agg_ts(
    ...
    df_events = pd.read_csv(
        os.path.join(data_directory_events, "StormEvents_2014_2024.csv")
    )
    ...
    df_state_ts_comb_hr = pd.merge(
        df_state_ts_events_hr, df_state_ts_power_hr, left_index=True, right_index=True
    )
    df_state_ts_comb_day = pd.merge(
        df_state_ts_events_day, df_state_ts_power_day, left_index=True, right_index=True
    )

    return df_state_ts_comb_hr, df_state_ts_comb_day


def plot_ts_events_power(
    df, event_types, start_year, start_month, start_day, end_year, end_month, end_day
):
    """
    ...
    for i, event in enumerate(event_types):
        ax1.plot(
            df_filtered.index,
            df_filtered[event],
            label=event.replace("event_count ", ""),
            color=colors[i % len(colors)],
            linewidth=2.0,
            alpha=0.7,
            linestyle="-.",
        )  # Thicker and semi-transparent lines with dashed-dot style
    ...
    ax2.plot(
        df_filtered.index,
        df_filtered["customers_out"],
        color="red",
        linewidth=2,
        linestyle="-",
    )  # Thicker line with dashed-dot style
    ...
    # Set the title
    plt.title(
        "Time Histories of Event Counts and Power Outages", fontsize=label_fontsize
    )
